-- write an sql query to find out percentage increase in covid cases versus cummulative cases as of proir month.
...
-- method 1
with cte as (
select month(record_date) as month, sum(cases_count) as monthly_cases_count
from covid_cases
group by month(record_date)
)

select cm.month,min(cm.monthly_cases_count),sum( pm.monthly_cases_count),min(cm.monthly_cases_count)*100.0/sum( pm.monthly_cases_count) as cumm_perc_change from cte cm
inner join cte pm on cm.month>pm.month
group by cm.month
order by month;
...
select *,
sum(monthly_cases_count) over(order by month rows between unbounded preceding and 1 preceding) as cumm_count,
monthly_cases_count*100/sum(monthly_cases_count) over(order by month rows between unbounded preceding and 1 preceding) as cumm_perc_change
from cte;
