...
-- select month(t1.order_date), count( distinct t2.order_id) from transactions t1
...
-- churn 
-- select * from transactions;
-- select month(t1.order_date), sum(case when t2.order_id is null then 1 else 0 end) from transactions t1
