...
Create table  Trips (id int, client_id int, driver_id int, city_id int,
status varchar(50), 
...
inner join Users u2 on t.driver_id=u2.users_id and u.banned = 'No')

select request_at, sum(case when status != 'completed' then 1 else 0 end)/ count(*) as cancellation_rate from cte
group by request_at
order by request_at;