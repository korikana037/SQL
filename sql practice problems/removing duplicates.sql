-- remove duplicates incase of same sorce, destination and distance and keep the first values only
...
left join cte c2 on
c1.source = c2.destination and c1.destination = c2.source
...
with cte as(
  select *,
  row_number() over(partition by distance order by distance) as rnk
  from city_distance
  order by distance
  )
