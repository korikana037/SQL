...
-- question 1 write a query to find total users in each segment and no of users who booked flight in april 2022
select segment, count(distinct ut.User_id) as total_users_count,
count( distinct case when month(bt.Booking_date)=4 and bt.Line_of_business= 'Flight' then ut.User_id end) as user_who_booked_flight_in_apr2022
from 
user_table ut left join booking_table bt on ut.User_id=bt.User_id
group by segment;

-- write a query to identify users whose first booking is hostel booking
select * from(
select *,
...
-- write a query to calculate the days between first and last booking of a User_id
select User_id, datediff(max(Booking_date),min(Booking_date)) as no_of_days from booking_table
group by User_id;

-- write a query to calculate the no.of flight and hotel bookings in each of the user segment for the year 2022 
...
from booking_table bt inner join user_table ut on bt.User_id=ut.User_id
group by ut.segment;
